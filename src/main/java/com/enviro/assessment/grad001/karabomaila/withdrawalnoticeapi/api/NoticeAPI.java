...
public class NoticeAPI {
    ...
    public ResponseEntity createNotice(@RequestParam(required = true) long investorId,
        ...
        if (productType.equals("SAVINGS") || productType.equals("RETIREMENT")){
            if (investor.isPresent() && investor.get().getAge() <= 65){
                throw new Exception("Not in a retirement age");
            }
            else if (investor.isPresent() && investor.get().isAmountGreater(productType, amount)){
